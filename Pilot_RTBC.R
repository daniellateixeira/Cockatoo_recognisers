# -----------------------------------------------------------------#
# CALL RECOGNISER: SOUTH-EASTERN RED-TAILED BLACK-COCKATOO
...
stackname <- "RTBC_templates_pilot_full_" # Name for full list of all templates tested
load(".\\RTBC_templates_pilot.Rdata")
...
temp1 <- btemps["-29_Flight3_Alan.wav"] # cutoff 17.6
temp2 <- btemps["-30_Flight2_Fourbird.wav"] # cutoff 19.2
#temp3 <- btemps["-28_Perch1_Loner.wav"] # cut off 17.8

runnertemps <- combineBinTemplates(temp1, temp2) # In brackets, the template number in the btemps stack
runnertemps
templateCutoff(runnertemps) <- c(17.6, 19.2) # Update their score cut off if necessary (check against aggregator)
runnertemps
